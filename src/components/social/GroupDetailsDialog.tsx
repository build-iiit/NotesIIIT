...
export function GroupDetailsDialog({ isOpen, onClose, groupId, defaultTab ='MEMBERS' }: GroupDetailsDialogProps) {
 ...
 className={`pb-2 text-sm font-bold transition-colors relative ${activeTab ==='MEMBERS' ?'text-primary' :'text-gray-500 hover:text-gray-800 dark:hover:text-gray-200'}`}
 >
 Members
 {activeTab ==='MEMBERS' && <span className="absolute bottom-0 left-0 w-full h-0.5 bg-primary rounded-full" />}
 </button>
 <button
 onClick={() => setActiveTab('FILES')}
 className={`pb-2 text-sm font-bold transition-colors relative ${activeTab ==='FILES' ?'text-primary' :'text-gray-500 hover:text-gray-800 dark:hover:text-gray-200'}`}
 >
 Files
 {activeTab ==='FILES' && <span className="absolute bottom-0 left-0 w-full h-0.5 bg-primary rounded-full" />}
 </button>
 {isAdmin && (
 <button
 onClick={() => { setActiveTab('SETTINGS'); setEditName(group?.name ||""); }}
 className={`pb-2 text-sm font-bold transition-colors relative ${activeTab ==='SETTINGS' ?'text-primary' :'text-gray-500 hover:text-gray-800 dark:hover:text-gray-200'}`}
 >
 Settings
 {activeTab ==='SETTINGS' && <span className="absolute bottom-0 left-0 w-full h-0.5 bg-primary rounded-full" />}
 </button>
 )}
 </div>
