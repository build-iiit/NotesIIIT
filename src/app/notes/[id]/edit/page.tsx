...
export default function EditNotePage({ params }: { params: Promise<{ id: string }> }) {
 ...
 const { data: tags, refetch: refetchTags } = api.notes.tags.getAll.useQuery();
...
"1-1","1-2","2-1","2-2","3-1","3-2","4-1","4-2","5-1","5-2"
 ...
 className="w-full flex items-center justify-between px-4 py-3 rounded-xl bg-white/50 dark:bg-white/10 border border-white/40 dark:border-white/10 hover:border-primary/50 transition-colors"
 >
 ...
 {isTagDropdownOpen && (
 <div className="absolute z-[100] w-full mt-2 bg-white/80 dark:bg-zinc-900/80 rounded-2xl shadow-[0_16px_48px_rgba(0,0,0,0.2)] border border-white/40 dark:border-white/10 max-h-72 overflow-y-auto">
 ...
 className="flex-1 px-4 py-2.5 rounded-xl bg-white/50 dark:bg-white/10 border border-white/40 dark:border-white/10 text-sm outline-none focus:border-primary/60 focus:ring-2 focus:ring-primary/20 transition-all text-gray-900 dark:text-white placeholder:text-gray-400"
 />
 <button
 type="button"
 onClick={async () => {
 ...
 className="bg-primary text-primary-foreground px-4 py-2 rounded-lg disabled:opacity-50 hover:bg-primary/80 transition-all font-medium"
 >
